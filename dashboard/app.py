...
st.sidebar.markdown(
    "Adjust the timeframe and scope to see the impact in real time.")
...
pct_late = total_late / \
    df_filtered['delay_at_checkout_in_minutes'].notna().sum() * 100
...
with col_g1:
    ...
    late_data = df_filtered[df_filtered['delay_at_checkout_in_minutes']
    ...
    fig1.add_vline(
        x=late_data.median(),
        line_color="red",
        annotation_text=f"Median: {late_data.median():.0f}min")
    fig1.add_vline(
        x=threshold,
        line_color="blue",
        line_dash="dash",
...
df_with_prev = df_filtered[df_filtered['previous_ended_rental_id'].notna(
)].copy()
...
for t in thresholds_range:
    blocked_count = len(
        df_filtered[df_filtered['time_delta_with_previous_rental_in_minutes'] < t])
    solved_count = len(df_merged[(df_merged['was_impacted'] == True) & (
...
with col_r1:
    ...
    for scope_type in ['connect', 'mobile']:
        ...
        for t in [30, 60, 120, 180, 240]:
            blocked_rev = len(
                df_scope[df_scope['time_delta_with_previous_rental_in_minutes'] < t])
            scope_results.append({'Scope': scope_type,
                                  'Threshold': f"{t}min",
                                  'Blocked': blocked_rev,
                                  '% blocked': blocked_rev / len(df_scope) * 100})
...
st.header("💰 Prix optimal — Prédiction")
mileage = st.number_input("Kilométrage", value=50000)
engine_power = st.number_input("Puissance moteur", value=120)
# ... autres inputs
if st.button("Prédire le prix"):
    response = requests.post("https://Dreipfelt-getaround-api.hf.space/predict", 
                            json={"input": [[mileage, engine_power, ...]]})
    st.success(f"Prix suggéré : {response.json()['prediction'][0]} €/jour")